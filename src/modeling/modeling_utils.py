...
def load_data():
    ...
    train = pd.read_csv('../data/csvs/train_sample.csv')
    test = pd.read_csv('../data/csvs/test_sample.csv')
    return train, test
...
def train_model(model,
    ...
    log_df = pd.DataFrame({'epoch':[],
                          ...
                          'train_acc': [],
                          'val_acc': []})
    log_filename = f'{save_path.split("_")[0]}_logs.csv'
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            # print(type(train_loss_history[-1]), type(val_loss_history[-1]), type(train_acc_history[-1]), type(val_acc_history[-1]))
...
def load_model(path, base, old_classes, new_classes, device):
    ...
    model.fc = nn.Linear(num_ftrs, old_classes)
    model.load_state_dict(torch.load(path)['model_state_dict'])
    model.fc = nn.Linear(num_ftrs, new_classes)
